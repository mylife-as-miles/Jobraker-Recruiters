...
This file provides context for AI coding agents working on the Jobraker Recruiter monorepo.
...
- Config file: `~/.jobraker-recruiter/config/models.json`
- Schema: `{ provider: { flavor, apiKey?, baseURL?, headers? }, model: string }`
- Models catalog cache: `~/.jobraker-recruiter/config/models.dev.json` (OpenAI/Anthropic/Google only)
