...
import z from "zod";
import { JobrakerRecruiterApi } from "./api.js";
...
export function JobrakerRecruiterTui({ serverUrl }: { serverUrl: string }) {
    const api = useMemo(() => new JobrakerRecruiterApi({ baseUrl: serverUrl }), [serverUrl]);
    const { exit } = useApp();
...
function Header({
    ...
    return (
        <Box flexDirection="column">
            <Text>
                <Text color="cyanBright">Jobraker Recruiter X</Text> chat · Server {serverUrl}
            </Text>
            <Text>
                {state === "connecting" && (
                    <>
