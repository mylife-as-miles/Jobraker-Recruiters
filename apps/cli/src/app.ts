...
export async function modelConfig() {
    ...
    try {
        ...
        const defaultModels: Record<z.infer<typeof Flavor>, string> = {
            "jobraker-recruiter [free]": "google/gemini-3-pro-preview",
            openai: "gpt-5.1",
            aigateway: "gpt-5.1",
        ...
        if (selectedFlavor !== "jobraker-recruiter [free]") {
        ...
        if (selectedFlavor !== "ollama" && selectedFlavor !== "jobraker-recruiter [free]") {
            let autopickText = "";
...
export async function importExample(exampleName?: string, filePath?: string) {
    ...
    // Display next steps
    console.log(`\nRun: jobraker-recruiter-x --agent ${entryAgent}`);
}
