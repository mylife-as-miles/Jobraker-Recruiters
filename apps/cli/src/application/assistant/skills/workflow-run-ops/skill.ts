...
Package of repeatable commands for running agents, inspecting agent run history under ~/.jobraker-recruiter/runs, and managing cron schedules. Load this skill whenever a user asks about running agents, execution history, paused runs, or scheduling.
...
**To run any agent**:
\`\`\`bash
jobraker-recruiter-x --agent <agent-name>
\`\`\`

**With input**:
\`\`\`bash
jobraker-recruiter-x --agent <agent-name> --input "your input here"
\`\`\`

**Non-interactive** (for automation/cron):
\`\`\`bash
jobraker-recruiter-x --agent <agent-name> --input "input" --no-interactive
\`\`\`
...
Operate from ~/.jobraker-recruiter (Jobraker Recruiter tools already set this as the working directory). Use executeCommand with the sample Bash snippets below, modifying placeholders as needed.
...
1. **List all runs**
   ...
   ls ~/.jobraker-recruiter/runs
   

2. **Filter by agent**
   
   grep -rl '"agent":"<agent-name>"' ~/.jobraker-recruiter/runs | xargs -n1 basename | sed 's/\.jsonl$//' | sort -r
...
4. **Show runs waiting for human input**
   ...
   awk 'FNR==1{if (NR>1) print fn, last; fn=FILENAME} {last=$0} END{print fn, last}' ~/.jobraker-recruiter/runs/*.jsonl | grep 'pause-for-human-input' | awk '{print $1}'
...
2. **Schedule an agent to run periodically**
   \`\`\`bash
   (crontab -l 2>/dev/null; echo '0 10 * * * cd /path/to/cli && jobraker-recruiter-x --agent <agent-name> --input "input" --no-interactive >> ~/.jobraker-recruiter/logs/<agent-name>.log 2>&1') | crontab -
   \`\`\`
   
   Example (runs daily at 10 AM):
   \`\`\`bash
   (crontab -l 2>/dev/null; echo '0 10 * * * cd ~/jobraker-recruiter-V2/apps/cli && jobraker-recruiter-x --agent podcast_workflow --no-interactive >> ~/.jobraker-recruiter/logs/podcast.log 2>&1') | crontab -
   \`\`\`
