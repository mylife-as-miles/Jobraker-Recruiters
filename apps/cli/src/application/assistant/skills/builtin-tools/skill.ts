...
Load this skill when creating or modifying agents that need access to Jobraker Recruiter's builtin tools (shell execution, file operations, etc.).
...
**Security note:** Commands are filtered through \`.jobraker-recruiter/config/security.json\`. Populate this file with allowed command names (array or dictionary entries). Any command not present is blocked and returns exit code 126 so the agent knows it violated the policy.
...
While \`executeCommand\` is the most versatile, other builtin tools exist for specific Jobraker Recruiter operations (file management, agent inspection, etc.). These are primarily used by the Jobraker Recruiter copilot itself and are not typically needed in user agents. If you need file operations, consider using bash commands like \`cat\`, \`echo\`, \`tee\`, etc. through \`executeCommand\`.

### Copilot-Specific Builtin Tools

The Jobraker Recruiter copilot has access to special builtin tools that regular agents don't typically use. These tools help the copilot assist users with workspace management and MCP integration:
...
- \`loadSkill\` - Load a Jobraker Recruiter skill definition into context
...
3. Running the orchestrator with \`jobraker-recruiter-x --agent orchestrator_name\`
