...
function PageBody() {
  const [apiBase, setApiBase] = useState<string>("http://127.0.0.1:3002")
        ...
        if (selectedResource.kind === "agent") {
          ...
          if (isMarkdown) {
            ...
            const response = await fetch(
              `/api/jobraker-recruiter/agent?file=${encodeURIComponent(raw)}`
            );
            if (!response.ok) {
          ...
          if (lower.endsWith(".md") || lower.endsWith(".markdown")) {
            ...
            try {
              const response = await fetch(
                `/api/jobraker-recruiter/config?file=${encodeURIComponent(selectedResource.name)}`
              );
              if (!response.ok) {
                if (response.status === 404) {
          ...
          const local = await requestJson(
            `/api/jobraker-recruiter/run?file=${encodeURIComponent(selectedResource.name)}`
          );
          if (local?.parsed) {
  ...
  useEffect(() => {
    const loadAgents = async () => {
      try {
        const res = await fetch("/api/jobraker-recruiter/summary");
        if (!res.ok) return;
    ...
    try {
      if (selectedResource.kind === "agent") {
        if (artifactFileType === "markdown") {
          const response = await fetch(
            `/api/jobraker-recruiter/agent?file=${encodeURIComponent(selectedResource.name)}`,
            {
              method: "PUT",
            ...
            <Breadcrumb>
              <BreadcrumbList>
                <BreadcrumbItem className="hidden md:block">
                  <BreadcrumbLink href="#">Jobraker Recruiter X</BreadcrumbLink>
                </BreadcrumbItem>
