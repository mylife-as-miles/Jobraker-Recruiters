...
export function AppSidebar({ onSelectResource, ...props }: AppSidebarProps) {
  ...
  const [summary, setSummary] = React.useState<JobrakerRecruiterSummary>({
    agents: [],
    config: [],
    runs: [],
  })
  ...
  React.useEffect(() => {
    const load = async () => {
      try {
        ...
        console.error("Failed to load jobraker-recruiter summary", error)
      } finally {
        setLoading(false)
      }
    }
