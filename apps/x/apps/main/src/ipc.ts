...
export function setupIpcHandlers() {
  ...
  registerIpcHandlers({
    ...
    'account:getJobrakerRecruiter': async () => {
      const signedIn = await isSignedIn();
      if (!signedIn) {
        return { signedIn: false, accessToken: null, config: null };
      }

      const config = await getJobrakerRecruiterConfig();
    ...
    'inline-task:process': async (_event, args) => {
      return await processJobrakerRecruiterInstruction(args.instruction, args.noteContent, args.notePath);
    },
