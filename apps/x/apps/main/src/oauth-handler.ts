      ...
      if (!credentials?.clientId || !credentials?.clientSecret) {
        ...
        if (await isSignedIn()) {
          try {
            ...
            console.log('[OAuth] Started jobraker-recruiter-mode Google connect (browser opened to webapp)');
            return { success: true };
          } catch (error) {
            console.error('[OAuth] Failed to start jobraker-recruiter-mode Google connect:', error);
            return {
              success: false,
    ...
    const { server, port: boundPort } = await createAuthServer(
      ...
      async (callbackUrl) => {
        ...
        try {
          ...
          // For Jobraker Recruiter sign-in, ensure user + Stripe customer exist before
...
/**
 * Complete a jobraker-recruiter-mode Google connect: claim the tokens parked under
 ...
 * jobraker-recruiter://oauth/google/done?session=<state> URL.
 */
export async function completeJobrakerRecruiterGoogleConnect(state: string): Promise<void> {
  try {
    ...
    console.log('[OAuth] Jobraker Recruiter-mode Google connect complete');
  } catch (error) {
    console.error('[OAuth] Failed to complete jobraker-recruiter-mode Google connect:', error);
    emitOAuthEvent({
      provider: 'google',
      success: false,
...
export async function disconnectProvider(provider: string): Promise<{ success: boolean }> {
  try {
    ...
    // For jobraker-recruiter-mode Google, best-effort revoke at Google before clearing
    ...
    if (provider === 'google') {
      const connection = await oauthRepo.read(provider);
      if (connection.mode === 'jobraker-recruiter' && connection.tokens?.access_token) {
        try {
...
export async function disconnectGoogleIfScopesStale(): Promise<void> {
  try {
    ...
    // Best-effort revoke at Google for jobraker-recruiter-mode grants (mirrors disconnectProvider).
    if (connection.mode === 'jobraker-recruiter' && connection.tokens.access_token) {
      try {
        const revokeUrl = `https://oauth2.googleapis.com/revoke?token=${encodeURIComponent(connection.tokens.access_token)}`;
        const res = await fetch(revokeUrl, { method: 'POST', signal: AbortSignal.timeout(5000) });
