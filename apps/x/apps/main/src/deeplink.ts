...
export const DEEP_LINK_SCHEME = "jobraker-recruiter";
...
/**
 * Dispatch any jobraker-recruiter:// URL — chooses among action / oauth-completion /
 * navigation automatically. Use this from notification click handlers and
 * other URL entry points.
 *
 * OAuth completion (jobraker-recruiter://oauth/google/done?session=<state>) is handled
...
// --- OAuth completion (jobraker-recruiter-mode Google connect) ---
...
/**
 * Match jobraker-recruiter://oauth/google/done?session=<state>. Returns null for
...
async function dispatchOAuthCompletion(url: string): Promise<void> {
    ...
    // oauthEvent IPC that completeJobrakerRecruiterGoogleConnect emits.
    const win = mainWindowRef;
    ...
    const { completeJobrakerRecruiterGoogleConnect } = await import("./oauth-handler.js");
    await completeJobrakerRecruiterGoogleConnect(parsed.state);
}
