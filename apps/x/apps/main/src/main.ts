...
// Single-instance lock: route a second launch (e.g. clicking a jobraker-recruiter:// link)
// back into the existing process via the 'second-instance' event.
if (app.isPackaged && !app.requestSingleInstanceLock()) {
  console.error('[Main] Another Jobraker Recruiter instance is already running; exiting this process.');
  app.quit();
  process.exit(0);
}

// Register as the OS handler for jobraker-recruiter:// URLs.
...
app.whenReady().then(async () => {
  ...
  if (app.isPackaged) {
    updateElectronApp({
      updateSource: {
        ...
        repo: "jobraker-recruiter/jobraker-recruiter",
      },
  ...
  // start inline task service (@jobraker-recruiter: mentions)
  initInlineTasks();
