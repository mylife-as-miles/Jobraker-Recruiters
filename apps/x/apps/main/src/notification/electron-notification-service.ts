...
const JOBRAKER_RECRUITER_URL = /^jobraker-recruiter:\/\//i;
...
export class ElectronNotificationService implements INotificationService {
    ...
    notify({ title = "Jobraker Recruiter", message, link, actionLabel, secondaryActions }: NotifyInput): void {
        ...
        const openLink = (target: string | undefined) => {
            if (target && JOBRAKER_RECRUITER_URL.test(target)) {
                dispatchUrl(target);
