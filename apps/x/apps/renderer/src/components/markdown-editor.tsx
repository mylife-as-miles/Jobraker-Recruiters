...
import { JobrakerRecruiterMentionPopover } from './jobraker-recruiter-mention-popover'
import '@/styles/editor.css'

type JobrakerRecruiterMentionMatch = {
  range: { from: number; to: number }
}

type JobrakerRecruiterBlockEdit = {
...
export const MarkdownEditor = forwardRef<MarkdownEditorHandle, MarkdownEditorProps>(function MarkdownEditor({
  ...
  const [activeJobrakerRecruiterMention, setActiveJobrakerRecruiterMention] = useState<JobrakerRecruiterMentionMatch | null>(null)
  const [jobrakerRecruiterBlockEdit, setJobrakerRecruiterBlockEdit] = useState<JobrakerRecruiterBlockEdit | null>(null)
  const [jobrakerRecruiterAnchorTop, setJobrakerRecruiterAnchorTop] = useState<{ top: number; left: number; width: number } | null>(null)
  const jobrakerRecruiterBlockEditRef = useRef<JobrakerRecruiterBlockEdit | null>(null)
  ...
  const updateJobrakerRecruiterMentionState = useCallback(() => {
    if (!editor) return
    const { selection } = editor.state
    if (!selection.empty) {
      setActiveJobrakerRecruiterMention(null)
      setJobrakerRecruiterAnchorTop(null)
      return
    }
    ...
    // Match @jobraker-recruiter at a word boundary (preceded by nothing or whitespace)
    const match = textBefore.match(/(^|\s)@jobraker-recruiter$/)
    if (!match) {
      setActiveJobrakerRecruiterMention(null)
      setJobrakerRecruiterAnchorTop(null)
      return
    ...
    const triggerStart = textBefore.length - '@jobraker-recruiter'.length
    const from = selection.from - (textBefore.length - triggerStart)
    const to = selection.from
    setActiveJobrakerRecruiterMention({ range: { from, to } })
  ...
  const updateAtMentionState = useCallback(() => {
    ...
    // If the full "@jobraker-recruiter" is already typed, let updateJobrakerRecruiterMentionState handle it
    if (query === 'jobraker-recruiter') {
      setActiveAtMention(null)
      setAtAnchorPosition(null)
      return
    }
  ...
  useEffect(() => {
    if (!editor) return
    editor.on('update', updateJobrakerRecruiterMentionState)
    editor.on('selectionUpdate', updateJobrakerRecruiterMentionState)
    return () => {
      editor.off('update', updateJobrakerRecruiterMentionState)
      editor.off('selectionUpdate', updateJobrakerRecruiterMentionState)
    }
  }, [editor, updateJobrakerRecruiterMentionState])
  ...
  // When a tell-jobraker-recruiter block is clicked, compute anchor and open popover
  useEffect(() => {
    if (!jobrakerRecruiterBlockEdit || !editor) return
    const wrapper = wrapperRef.current
    if (!wrapper) return
    const coords = editor.view.coordsAtPos(jobrakerRecruiterBlockEdit.nodePos)
    const wrapperRect = wrapper.getBoundingClientRect()
    const proseMirrorEl = wrapper.querySelector('.ProseMirror') as HTMLElement | null
    const pmRect = proseMirrorEl?.getBoundingClientRect()
    setJobrakerRecruiterAnchorTop({
  ...
  }, [editor, jobrakerRecruiterBlockEdit])
  ...
  const handleJobrakerRecruiterAdd = useCallback(async (instruction: string) => {
    if (!editor) return

    if (jobrakerRecruiterBlockEdit) {
      // Editing existing taskBlock — update its data attribute
      const { nodePos } = jobrakerRecruiterBlockEdit
      ...
      setJobrakerRecruiterBlockEdit(null)
      jobrakerRecruiterBlockEditRef.current = null
      setJobrakerRecruiterAnchorTop(null)
      return
    }

    if (activeJobrakerRecruiterMention) {
      // Insert a temporary processing block
      const blockData: Record<string, unknown> = { instruction, processing: true }

      const insertFrom = activeJobrakerRecruiterMention.range.from
      const insertTo = activeJobrakerRecruiterMention.range.to
      ...
      try {
          ...
          if (notePath) {
            setTimeout(async () => {
              try {
                ...
                console.error('[JobrakerRecruiterAdd] Failed to write target tags:', err)
              }
            }, 500)
          }
        } else {
        ...
        console.error('[JobrakerRecruiterAdd] Processing failed:', error)
  ...
  }, [editor, activeJobrakerRecruiterMention, jobrakerRecruiterBlockEdit, frontmatter, onFrontmatterChange, notePath])

  const handleJobrakerRecruiterRemove = useCallback(() => {
    if (!editor || !jobrakerRecruiterBlockEdit) return
    const { nodePos } = jobrakerRecruiterBlockEdit
    const node = editor.state.doc.nodeAt(nodePos)
    if (node) {
      editor
  ...
  const atMentionOptions = useMemo(() => [
    { value: 'jobraker-recruiter', label: '@jobraker-recruiter', description: 'Research, schedule, or run tasks with AI' },
  ], [])
  ...
  const handleSelectAtMention = useCallback((value: string) => {
    ...
    if (value === 'jobraker-recruiter') {
      // Replace "@<partial>" with "@jobraker-recruiter" — this triggers updateJobrakerRecruiterMentionState
      editor
        .chain()
        .focus()
        .insertContentAt(
    ...
    <div className="tiptap-editor" onKeyDown={handleKeyDown}>
      <EditorToolbar
        ...
        onOpenLiveNote={notePath ? () => {
          window.dispatchEvent(new CustomEvent('jobraker-recruiter:open-live-note-panel', {
      ...
      <div className="editor-content-wrapper" ref={wrapperRef} onScroll={handleScroll}>
        ...
        <JobrakerRecruiterMentionPopover
          open={Boolean((activeJobrakerRecruiterMention || jobrakerRecruiterBlockEdit) && jobrakerRecruiterAnchorTop)}
          anchor={jobrakerRecruiterAnchorTop}
          initialText={jobrakerRecruiterBlockEdit?.existingText ?? ''}
          onAdd={handleJobrakerRecruiterAdd}
          onRemove={jobrakerRecruiterBlockEdit ? handleJobrakerRecruiterRemove : undefined}
          onClose={() => {
            setActiveJobrakerRecruiterMention(null)
            setJobrakerRecruiterBlockEdit(null)
            jobrakerRecruiterBlockEditRef.current = null
