...
export function ConnectorsPopover({ children, tooltip, open: openProp, onOpenChange, mode = "all" }: ConnectorsPopoverProps) {
        ...
        <div className="shrink-0">
          {state.isLoading ? (
            ...
            <Button
              ...
              className="h-7 px-2 text-xs"
            >
              {provider === 'jobraker-recruiter' ? 'Log Out' : 'Disconnect'}
            </Button>
          ) : (
            <Button
              variant="default"
            ...
            >
              {state.isConnecting ? (
                ...
                provider === 'jobraker-recruiter' ? 'Log In' : 'Connect'
              )}
            </Button>
          )}
        </div>
  ...
  const allConnected = isUnconnectedMode && !isJobrakerRecruiterUnconnected && !hasUnconnectedEmailCalendar && !hasUnconnectedMeetingNotes
              ...
              {c.providers.includes('jobraker-recruiter') && (() => {
                ...
                return (
                  <>
                    ...
                    {renderOAuthProvider('jobraker-recruiter', 'Jobraker Recruiter', <User className="size-4" />, 'Log in to your Jobraker Recruiter account')}
