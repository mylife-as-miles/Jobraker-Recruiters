...
interface BillingJobrakerRecruiterAccount {
  config?: {
    appUrl?: string | null
  } | null
}
...
export function BillingErrorDialog({ open, match, onOpenChange }: BillingErrorDialogProps) {
  ...
  useEffect(() => {
    if (!open) return
    window.ipc
      .invoke('account:getJobrakerRecruiter', null)
      .then((account: BillingJobrakerRecruiterAccount) => setAppUrl(account.config?.appUrl ?? null))
      .catch(() => {})
  }, [open])
