...
export function useConnectors(active: boolean) {
  ...
  const startConnect = useCallback(async (provider: string, credentials?: { clientId: string; clientSecret: string }) => {
    ...
    try {
      ...
      if (!result.success) {
        toast.error(result.error || (provider === 'jobraker-recruiter' ? 'Failed to log in to Jobraker Recruiter' : `Failed to connect to ${provider}`))
        setProviderStates(prev => ({
          ...prev,
      ...
      toast.error(provider === 'jobraker-recruiter' ? 'Failed to log in to Jobraker Recruiter' : `Failed to connect to ${provider}`)
      setProviderStates(prev => ({
        ...prev,
        [provider]: { ...prev[provider], isConnecting: false }
      }))
  ...
  const handleDisconnect = useCallback(async (provider: string) => {
    ...
    try {
      ...
      if (result.success) {
        ...
        toast.success(provider === 'jobraker-recruiter' ? 'Logged out of Jobraker Recruiter' : `Disconnected from ${displayName}`)
        setProviderStates(prev => ({
          ...prev,
          [provider]: {
            isConnected: false,
        ...
        toast.error(provider === 'jobraker-recruiter' ? 'Failed to log out of Jobraker Recruiter' : `Failed to disconnect from ${provider}`)
        setProviderStates(prev => ({
          ...prev,
