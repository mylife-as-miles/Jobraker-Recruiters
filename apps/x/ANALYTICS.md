...
- **Anonymous distinct_id** = `installationId` from `~/.jobraker-recruiter/config/installation.json` (auto-generated on first run; see `packages/core/src/analytics/installation.ts`).
...
- **On jobraker-recruiter sign-in**: `posthog.identify(jobraker-recruiterUserId)` runs in **both** processes.
...
- **On every app startup**: main re-identifies if jobraker-recruiter tokens exist (`packages/core/src/analytics/identify.ts`, called from `apps/main/src/main.ts` whenReady). Idempotent — PostHog merges person properties on duplicate identifies. This catches users who installed before analytics existed, and refreshes person properties (plan/status) on every launch.
- **On jobraker-recruiter sign-out**: `posthog.reset()` in both processes; future events resolve to the installation_id again.
...
| `provider` | string | `jobraker-recruiter` = cloud LLM gateway; otherwise the BYOK provider (`openai`, `anthropic`, `ollama`, etc.) |
| `input_tokens` | number | |
...
| `knowledge_sync` | `inline_task_run` | yes | Inline `@jobraker-recruiter` task execution (two call sites) | `packages/core/src/knowledge/inline_tasks.ts:471, 552` (createRun) |
...
Emitted when jobraker-recruiter OAuth completes. Properties: `plan`, `status` (subscription state from `/v1/me`).
...
Emitted on jobraker-recruiter disconnect. No properties. Followed immediately by `posthog.reset()`.
...
| `api_url` | both processes (init + identify) | Distinguishes prod / staging / custom — assign meaning in PostHog dashboard. `https://api.x.jobraker-recruiter.com` = production |
| `app_version` | both processes (init + identify) | Electron app version; also included automatically on every event |
| `signed_in` | renderer | `true` while jobraker-recruiter OAuth is connected |
| `{provider}_connected` | renderer | One of `gmail`, `calendar`, `slack`, `jobraker-recruiter` |
...
`installationId`: stored in `~/.jobraker-recruiter/config/installation.json`, generated on first run.
