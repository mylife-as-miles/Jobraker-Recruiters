 ...
 * to the native jobraker-recruiter-mode Google OAuth flow.
 *
 * Triggered by the renderer on app launch and after Jobraker Recruiter sign-in. The
...
export async function qualifyAndDisconnectComposioGoogle(): Promise<{ shouldShow: boolean }> {
    ...
    // Rule 1 — must be signed in to Jobraker Recruiter
    if (!(await isSignedIn())) {
        return { shouldShow: false };
    }

    // Rule 3 — already on native jobraker-recruiter-mode Google → silently mark dismissed
    ...
    if (googleConnection.tokens && googleConnection.mode === 'jobraker-recruiter') {
        markDismissed();
        return { shouldShow: false };
    }
