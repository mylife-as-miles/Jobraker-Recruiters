...
export interface EventConsumer {
     ...
     */
    findCandidates(event: JobrakerRecruiterEvent, targets: EventConsumerTarget[]): Promise<string[]>;

    /** Fire the consumer's agent on a single candidate id. */
    fireCandidate(event: JobrakerRecruiterEvent, id: string): Promise<EventConsumerFireResult>;
}
