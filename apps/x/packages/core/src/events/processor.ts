...
import type { JobrakerRecruiterEvent, ConsumerResult } from '@x/shared/dist/events.js';
...
function moveEventToDone(filename: string, enriched: JobrakerRecruiterEvent): void {
 ...
 */
function migrateLegacyTarget(event: JobrakerRecruiterEvent): JobrakerRecruiterEvent {
...
async function processOneEvent(filename: string): Promise<void> {
    ...
    if (registeredConsumers.length === 0) {
        ...
        const enriched: JobrakerRecruiterEvent = {
            ...event,
            processedAt: new Date().toISOString(),
            consumers: {},
        };
