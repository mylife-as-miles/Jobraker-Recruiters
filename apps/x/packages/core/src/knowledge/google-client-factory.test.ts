...
beforeEach(() => {
  ...
  mockOAuthRepo = {
    read: vi.fn(async () => ({ tokens: storedTokens, mode: 'jobraker-recruiter' as const })),
...
describe('GoogleClientFactory.getClient', () => {
  ...
  it('returns cached client when tokens are not expired', async () => {
    ...
    mockOAuthRepo.read = vi.fn(async () => ({ tokens: storedTokens, mode: 'jobraker-recruiter' as const }));
