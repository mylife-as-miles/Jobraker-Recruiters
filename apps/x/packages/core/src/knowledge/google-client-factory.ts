...
type Mode = 'byok' | 'jobraker-recruiter';
 ...
 *   - `jobraker-recruiter` signed-in user; client_id+secret never on the desktop;
 ...
 *
 * **Auto-refresh disabled in jobraker-recruiter mode:** google-auth-library's
 * OAuth2Client will, on a 401 from a Google API call, try to refresh using
 * the refresh_token + client secret it has on hand. In jobraker-recruiter mode we have
...
export class GoogleClientFactory {
    ...
    private static async getClientInner(): Promise<OAuth2Client | null> {
        ...
        // BYOK needs an openid-client Configuration for local refresh; jobraker-recruiter doesn't.
        if (mode === 'byok') {
            try {
    ...
    private static async refreshAndBuild(tokens: OAuthTokens, mode: Mode): Promise<OAuth2Client | null> {
        ...
        try {
            ...
            // Walk cause chain so we can see e.g. `Not signed into Jobraker Recruiter`
    ...
    private static async buildAndCacheClient(tokens: OAuthTokens, mode: Mode): Promise<OAuth2Client> {
        ...
        const client = mode === 'jobraker-recruiter'
            ? this.createJobrakerRecruiterClient(tokens)
    ...
    /**
     * Initialize cached configuration for BYOK mode (jobraker-recruiter doesn't need it).
     */
    ...
    /**
     * Jobraker Recruiter OAuth2Client — no client_id/secret, no refresh_token.
     ...
     */
    private static createJobrakerRecruiterClient(tokens: OAuthTokens): OAuth2Client {
