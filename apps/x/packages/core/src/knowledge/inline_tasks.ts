...
/**
 * Parse the tell-jobraker-recruiter block content (JSON format).
 * Returns { instruction, schedule } or null if not valid JSON.
 * Also supports legacy @jobraker-recruiter format.
 */
function parseBlockContent(contentLines: string[]): { instruction: string; schedule: InlineTaskSchedule | null; lastRunAt: string | null; targetId: string | null } | null {
    ...
    try {
        ...
        // Legacy format: @jobraker-recruiter lines + optional schedule: JSON line
    }

    // Legacy fallback: parse @jobraker-recruiter instruction and schedule: line
    ...
    const firstJobrakerRecruiterLine = instructionLines.find(l => l.trim().startsWith('@jobraker-recruiter'));
    const rawInstruction = firstJobrakerRecruiterLine?.trim() ?? instructionLines.join('\n').trim();
    const instruction = rawInstruction.replace(/^@jobraker-recruiter:?\s*/, '');
...
/**
 * Find ```tell-jobraker-recruiter code blocks in a note body and return tasks that are pending execution.
 */
function findPendingTasks(body: string): InlineTask[] {
    ...
    while (i < lines.length) {
        ...
        if (trimmed.startsWith('```task') || trimmed.startsWith('```tell-jobraker-recruiter')) {
            const startLine = i;
            i++;
...
/**
 * Insert the agent result below the tell-jobraker-recruiter code block in the body.
...
/**
 * Determine if a note has any "live" tell-jobraker-recruiter tasks.
 * A task is live if:
    ...
    for (const filePath of allFiles) {
        ...
        if (tasks.length === 0) {
            ...
            if (!live) {
                ...
                // Remove jobraker-recruiter_tasks if present (legacy cleanup)
                delete fields['jobraker-recruiter_tasks'];
        ...
        // Update frontmatter — only manage live_note, remove legacy jobraker-recruiter_tasks
...
/**
 * Process a @jobraker-recruiter instruction via the inline task agent.
...
export async function processJobrakerRecruiterInstruction(
    instruction: string,
    noteContent: string,
    notePath: string,
): Promise<{
    ...
    const message = [
        `Process the following @jobraker-recruiter instruction from the note "${notePath}":`,
        '',
    ...
    const scheduleMarkerRegex = /<!--jobraker-recruiter-schedule:([\s\S]*?)-->/;
    const scheduleMatch = rawResponse.match(scheduleMarkerRegex);

    // Parse out the instruction marker if present
    const instructionMarkerRegex = /<!--jobraker-recruiter-instruction:([\s\S]*?)-->/;
