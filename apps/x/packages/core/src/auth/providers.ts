...
export async function getProviderConfig(providerName: string): Promise<ProviderConfigEntry> {
  ...
  if (providerName === 'jobraker-recruiter') {
    const jobrakerRecruiterConfig = await getJobrakerRecruiterConfig();
    config.discovery = {
      mode: 'issuer',
      issuer: `${jobrakerRecruiterConfig.supabaseUrl}/auth/v1/.well-known/oauth-authorization-server`,
    }
  }
  return config;
}
