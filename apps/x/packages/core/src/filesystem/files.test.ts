...
beforeEach(async () => {
  tmpDir = await fs.mkdtemp(path.join(os.tmpdir(), "jobraker-recruiter-files-test-"));
  workspaceDir = path.join(tmpDir, "workspace");
  process.env.JOBRAKER_RECRUITER_WORKDIR = workspaceDir;
  vi.resetModules();
...
afterEach(async () => {
  delete process.env.JOBRAKER_RECRUITER_WORKDIR;
...
describe("filesystem files", () => {
  it("resolves relative paths inside JOBRAKER_RECRUITER_WORKDIR", async () => {
  ...
  it("expands home-relative paths", async () => {
    ...
    const resolved = files.resolveFilePath("~/jobraker-recruiter-test.txt");

    expect(resolved.resolvedPath).toBe(path.join(os.homedir(), "jobraker-recruiter-test.txt"));
    expect(resolved.isInsideWorkspace).toBe(false);
  });
