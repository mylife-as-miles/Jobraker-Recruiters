...
const APP_VERSION = (process.env.JOBRAKER_RECRUITER_APP_VERSION ?? process.env.npm_package_version ?? '').trim();
...
export function identify(userId: string, properties?: Record<string, unknown>): void {
  ...
  if (!ph) return;
  try {
    // Alias the anonymous installation ID to the jobraker-recruiter user ID so historical
