...
- Extract user-provided values into the correct fields (e.g., "jobraker-recruiter/jobraker-recruiter" → \`owner: "jobraker-recruiter", repo: "jobraker-recruiter"\`)
- For tools with empty \`properties: {}\`, pass \`arguments: {}\`
- For tools with required fields, pass all of them

### Example: GitHub Issues

User says: "Get me the open issues on jobraker-recruiter/jobraker-recruiter"

1. \`composio-search-tools({ query: "list issues", toolkitSlug: "github" })\`
   → finds \`GITHUB_ISSUES_LIST_FOR_REPO\` with required: ["owner", "repo"]
2. \`composio-execute-tool({ toolSlug: "GITHUB_ISSUES_LIST_FOR_REPO", toolkitSlug: "github", arguments: { owner: "jobraker-recruiter", repo: "jobraker-recruiter", state: "open", per_page: 100 } })\`
