...
Coding agents operate on **arbitrary file paths** (including paths outside the Jobraker Recruiter workspace root, like \`G:/4th sem/CN\` or \`~/projects/foo\`). Do NOT raise "outside workspace" concerns, and do NOT fall back to your own \`executeCommand\` (PowerShell / bash) or workspace file tools to do code work yourself.
...
ask-human({
  ...
  options: [
    ...
    "Use code mode (Codex)",
    "Continue with default Jobraker Recruiter"
  ]
})
\`\`\`
...
- "Continue with default Jobraker Recruiter" → ABANDON this skill. Handle the request yourself using your own tools (workspace file tools, \`executeCommand\` shell, etc.). The rest of this skill does not apply for this turn.
...
- If the "# Code Mode (Active)" block gives a session name (e.g. \`jobraker-recruiter-<runId>\`), use that exact name.
...
- Only add your own explanation if the command failed (non-zero exit):
  ...
  - "command not found" / agent not installed, or an auth/sign-in error — the agent isn't set up. Tell the user to install or sign in to the agent via **Settings → Code Mode**, where Jobraker Recruiter shows the install and sign-in status.
