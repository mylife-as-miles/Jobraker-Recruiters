...
### Parameters
- **\`title\`** (optional, defaults to \`"Jobraker Recruiter"\`) — bold headline at the top.
...
- **\`link\`** (optional) — URL to open when the user clicks the notification. Two kinds accepted:
  ...
  - **\`jobraker-recruiter://...\`** — opens a view inside Jobraker Recruiter (see deep links below)
  - If omitted, clicking the notification focuses the Jobraker Recruiter app.
...
Deep link into a Jobraker Recruiter note:
\`\`\`json
{
  "message": "Daily brief is ready",
  "link": "jobraker-recruiter://open?type=file&path=knowledge/Daily/2026-04-25.md"
}
\`\`\`

## Deep links: \`jobraker-recruiter://\`

Use these as the \`link\` parameter to land the user on a specific view in Jobraker Recruiter instead of an external site. URL-encode paths/names that contain spaces or special characters.

| Target | Format | Example |
|---|---|---|
| Open a file | \`jobraker-recruiter://open?type=file&path=<workspace-relative path>\` | \`jobraker-recruiter://open?type=file&path=knowledge/People/Acme.md\` |
| Open chat | \`jobraker-recruiter://open?type=chat\` (optional \`&runId=<id>\`) | \`jobraker-recruiter://open?type=chat&runId=abc123\` |
| Knowledge graph | \`jobraker-recruiter://open?type=graph\` | — |
| Background task view | \`jobraker-recruiter://open?type=task&name=<task-name>\` | \`jobraker-recruiter://open?type=task&name=daily-brief\` |
| Suggested topics | \`jobraker-recruiter://open?type=suggested-topics\` | — |
