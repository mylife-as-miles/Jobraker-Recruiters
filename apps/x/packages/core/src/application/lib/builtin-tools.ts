...
export const BuiltinTools: z.infer<typeof BuiltinToolsSchema> = {
    loadSkill: {
        description: "Load a Jobraker Recruiter skill definition into context by fetching its guidance string",
    ...
    'file-remove': {
        description: 'Remove a file or directory. Files are moved to the Jobraker Recruiter trash by default for safety.',
        inputSchema: z.object({
    ...
    'composio-execute-tool': {
        description: 'Execute a Composio tool by its slug. You MUST pass the arguments field with all required parameters from the search results inputSchema. Example: composio-execute-tool({ toolSlug: "GITHUB_ISSUES_LIST_FOR_REPO", toolkitSlug: "github", arguments: { owner: "jobraker-recruiter", repo: "jobraker-recruiter", state: "open", per_page: 100 } })',
        inputSchema: z.object({
        ...
        execute: async ({ toolSlug, toolkitSlug, arguments: args }: { toolSlug: string; toolkitSlug: string; arguments?: Record<string, unknown> }) => {
            ...
            try {
                return await executeComposioAction(toolSlug, {
                    ...
                    user_id: 'jobraker-recruiter-user',
                    version: 'latest',
                    arguments: toolArgs,
                });
            } catch (error) {
    ...
    'notify-user': {
        description: "Show a native OS notification to the user. Clicking the notification opens the provided link in the default browser, or focuses the Jobraker Recruiter app if no link is given.",
        inputSchema: z.object({
            title: z.string().min(1).max(120).optional().describe("Bold headline shown at the top of the notification. Defaults to 'Jobraker Recruiter'."),
            message: z.string().min(1).describe("Body text of the notification."),
            link: z.string().url().refine((v) => /^(https?|jobraker-recruiter):\/\//i.test(v), {
                message: "link must be an http(s):// or jobraker-recruiter:// URL",
            }).optional().describe("Optional URL opened when the user clicks the notification. Accepts http(s):// (opens in browser) or jobraker-recruiter:// (opens a view inside Jobraker Recruiter — see the notify-user skill for deep-link shapes)."),
            ...
            secondaryActions: z.array(z.object({
                ...
                link: z.string().url().refine((v) => /^(https?|jobraker-recruiter):\/\//i.test(v), {
                    message: "secondary action link must be an http(s):// or jobraker-recruiter:// URL",
                }),
