...
function resolveWorkDir(): string {
    const configured = process.env.JOBRAKER_RECRUITER_WORKDIR;
    if (!configured) {
        return path.join(homedir(), ".jobraker-recruiter");
    }
...
// Allow override via JOBRAKER_RECRUITER_WORKDIR env var for standalone pipeline usage.
