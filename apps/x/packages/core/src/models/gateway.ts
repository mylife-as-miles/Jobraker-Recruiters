...
const authedFetch: typeof fetch = async (input, init) => {
    ...
    if (ctx?.useCase) headers.set('x-jobraker-recruiter-use-case', ctx.useCase);
    if (ctx?.subUseCase) headers.set('x-jobraker-recruiter-sub-use-case', ctx.subUseCase);
    if (ctx?.agentName) headers.set('x-jobraker-recruiter-agent-name', ctx.agentName);
...
export function getGatewayProvider(): ProviderV2 {
    return createOpenRouter({
        ...
        apiKey: 'managed-by-jobraker-recruiter',
        fetch: authedFetch,
    });
}
