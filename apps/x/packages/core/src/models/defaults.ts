...
const SIGNED_IN_DEFAULT_PROVIDER = "jobraker-recruiter";
 ...
 *
 * - "jobraker-recruiter" → gateway provider (auth via OAuth bearer; no creds field).
...
export async function resolveProviderConfig(name: string): Promise<z.infer<typeof LlmProvider>> {
    if (name === "jobraker-recruiter") {
        return { flavor: "jobraker-recruiter" };
    }
    const repo = container.resolve<IModelConfigRepo>("modelConfigRepo");
    const cfg = await repo.getConfig();
