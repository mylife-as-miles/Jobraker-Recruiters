...
function isCopilotLikeAgent(agentName: string | null | undefined): boolean {
    return agentName === 'copilot' || agentName === 'jobraker-recruiter-x';
}
...
export async function loadAgent(id: string): Promise<z.infer<typeof Agent>> {
    if (id === "copilot" || id === "jobraker-recruiter-x") {
...
export async function* streamAgent({
    ...
    while (true) {
        ...
        if (state.agentName === 'copilot' || state.agentName === 'jobraker-recruiter-x') {
...
3. **Workspace-specific operations.** Anything that obviously belongs in the Jobraker Recruiter workspace (config files, MCP servers, agent schedules, etc.) stays in the workspace, not the work directory.
...
Do not announce the work directory unless it's relevant. Just use it.`;
        ...
        if (codeMode) {
            ...
            const sessionName = `jobraker-recruiter-${runId}`;
