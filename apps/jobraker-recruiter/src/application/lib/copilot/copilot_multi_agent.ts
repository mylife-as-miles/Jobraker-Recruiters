...
function findUsingJobrakerRecruiterDocsDir(): string | null {
  const candidates = [
    path.resolve(process.cwd(), '../docs/docs/using-jobraker-recruiter'),
    path.resolve(process.cwd(), 'apps/docs/docs/using-jobraker-recruiter'),
  ];
...
function collectDocsSummaries(): string {
  const dir = findUsingJobrakerRecruiterDocsDir();
  if (!dir) return '';
...
const USING_JOBRAKER_RECRUITER_DOCS = collectDocsSummaries();

// Inject auto-loaded docs, if available
export const COPILOT_INSTRUCTIONS_MULTI_AGENT_WITH_DOCS =
  COPILOT_INSTRUCTIONS_MULTI_AGENT.replace('{USING_JOBRAKER_RECRUITER_DOCS}', USING_JOBRAKER_RECRUITER_DOCS);
