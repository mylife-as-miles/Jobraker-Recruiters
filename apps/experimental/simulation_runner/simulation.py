...
from jobraker_recruiter import Client, StatefulChat

openai_client = OpenAI()
MODEL_NAME = "gpt-4.1"
JOBRAKER_RECRUITER_API_HOST = os.environ.get("JOBRAKER_RECRUITER_API_HOST", "http://127.0.0.1:3000").strip()

async def simulate_simulation(
    scenario: TestScenario,
    profile_id: str,
    pass_criteria: str,
    jobraker_recruiter_client: Client,
    workflow_id: str,
    ...
    for _ in range(max_iterations):
        ...
        # Run Jobraker Recruiter chat in a thread if it's synchronous
        jobraker_recruiter_response = await loop.run_in_executor(
            None,
            lambda: support_chat.run(simulated_content)
        )

        messages.append({"role": "user", "content": jobraker_recruiter_response})
...
async def simulate_simulations(
    ...
    client = Client(
        host=JOBRAKER_RECRUITER_API_HOST,
